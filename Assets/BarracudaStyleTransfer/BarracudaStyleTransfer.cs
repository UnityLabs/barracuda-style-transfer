	...
	private void OnRenderImageNormal(RenderTexture source, RenderTexture destination)
	{
		if (useFramerateUpsampling == false)
		{
			...
			else
			{
				...
				O.ToRenderTexture(tempRT, 0, 0, Vector4.one, internalSetup.postNetworkColorBias);
			...
			else
			{
				if (currentFrame == 0)
				{
					if (firstWorker == false)
					{
						...
						Tensor O = worker.PeekOutput();
						O.ToRenderTexture(iFrame1, 0, 0, Vector4.one, internalSetup.postNetworkColorBias);
						O.Dispose();
						input.Dispose();
					}
					firstWorker = false;
